                ...
                <v-card>
                    <v-card-title>
                        <v-text-field v-model="search" label="Müşteri Ara" prepend-inner-icon="mdi-magnify" dense
                            hide-details :class="isMobile ? 'w-100' : ''" />
                        <v-spacer />
                        <v-btn color="primary" @click="dialogYeni = true">Yeni Müşteri</v-btn>
                    </v-card-title>
                    <div style="overflow-x:auto">
                        <v-data-table :headers="headers" :items="filteredCariler" :search="search" item-key="id" dense>
                            <template #item.actions="{ item }">
                                <v-btn icon="mdi-eye" @click="openDetay(item)"></v-btn>
                                <v-btn icon="mdi-pencil" @click="openEdit(item)"></v-btn>
                                <v-btn icon="mdi-delete" color="red" @click="deleteCari(item)"></v-btn>
                            </template>
                        </v-data-table>
                    </div>
                </v-card>
            ...
            <v-col cols="12" md="4" v-if="!isMobile">
                <v-card v-if="detayCari">
                    ...
                    <v-card-text>
                        ...
                        <v-btn color="info" class="mt-2" @click="exportExcel"
