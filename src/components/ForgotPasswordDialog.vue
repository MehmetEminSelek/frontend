    ...
    <v-dialog v-model="isOpen" max-width="400" persistent>
        <v-card class="pa-4">
            ...
            <v-card-text>
                <div v-if="step === 'email'" class="email-step">
                    ...
                    <v-form ref="emailForm" v-model="emailFormValid" @submit.prevent="sendResetEmail">
                        <v-text-field v-model="email" label="E-posta Adresi" type="email" variant="outlined"
                            prepend-inner-icon="mdi-email" :rules="emailRules" :error-messages="emailErrors" required
                            class="mb-3" />
                    </v-form>
                </div>
                ...
                <div v-else-if="step === 'code'" class="code-step">
                    ...
                    <v-form ref="codeForm" v-model="codeFormValid" @submit.prevent="verifyCode">
                        <v-text-field v-model="resetCode" label="Sıfırlama Kodu" variant="outlined"
                            prepend-inner-icon="mdi-key" :rules="codeRules" :error-messages="codeErrors" maxlength="6"
                            class="mb-3 text-center" style="font-size: 18px; letter-spacing: 2px;" />

                        <div class="text-center">
                            <v-btn variant="text" color="primary" size="small" @click="resendCode"
                ...
                <div v-else-if="step === 'password'" class="password-step">
                    ...
                    <v-form ref="passwordForm" v-model="passwordFormValid" @submit.prevent="resetPassword">
                        <v-text-field v-model="newPassword" label="Yeni Şifre"
                            ...
                            :error-messages="passwordErrors" required class="mb-3" />

                        <v-text-field v-model="confirmPassword" label="Şifre Tekrarı"
                            :type="showConfirmPassword ? 'text' : 'password'" variant="outlined"
                            prepend-inner-icon="mdi-lock-check"
                            :append-inner-icon="showConfirmPassword ? 'mdi-eye' : 'mdi-eye-off'"
                            @click:append-inner="showConfirmPassword = !showConfirmPassword"
                            :rules="confirmPasswordRules" :error-messages="confirmPasswordErrors" required
                            class="mb-3" />
                        ...
                        <div class="password-strength mb-3">
                            ...
                            <v-progress-linear :model-value="passwordStrength.value" :color="passwordStrength.color"
                                height="4" rounded />
                        </div>
                    </v-form>
                </div>
            ...
            <v-card-actions class="pt-0">
                ...
                <v-btn v-if="step === 'email'" color="primary" variant="flat" @click="sendResetEmail"
                    :loading="isLoading" :disabled="!emailFormValid">
                    Kod Gönder
                </v-btn>

                <v-btn v-else-if="step === 'code'" color="primary" variant="flat" @click="verifyCode"
                    :loading="isLoading" :disabled="!codeFormValid">
                    Kodu Doğrula
                </v-btn>

                <v-btn v-else-if="step === 'password'" color="primary" variant="flat" @click="resetPassword"
                    :loading="isLoading" :disabled="!passwordFormValid">
                    Şifreyi Güncelle
                </v-btn>

                <v-btn v-else-if="step === 'success'" color="primary" variant="flat" @click="goToLogin">
                    Giriş Sayfasına Dön
                </v-btn>
            </v-card-actions>
        </v-card>
